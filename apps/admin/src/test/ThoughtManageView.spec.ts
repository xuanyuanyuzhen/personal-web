  ...
  it('loads thought list and submits create form', async () => {
    const fetchMock = vi.fn((url: string, init?: RequestInit) => {
      ...
      if (url === '/api/tags/public?scope=THOUGHT') {
        return Promise.resolve(
          jsonResponse([{ color: null, id: 1, isEnabled: true, name: '鏃ュ父', scopes: ['THOUGHT'], slug: 'daily' }]),
        );
      }
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/admin/thoughts?page=1&pageSize=10', expect.any(Object));
    ...
    vm.form.tagNames = ['日常'];
    await vm.handleImageSelected(fileChangeEvent(new File(['image'], 'thought.png', { type: 'image/png' })));
    await flushPromises();
    ...
    const createCall = fetchMock.mock.calls.find(([url, init]) => url === '/api/admin/thoughts' && init?.method === 'POST');
