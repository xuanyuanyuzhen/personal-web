...
function albumsPayload() {
  return [{ coverUrl: null, description: null, id: 1, isEnabled: true, name: '春日', slug: 'spring', sortOrder: 0, status: 'PUBLISHED', visibility: 'PUBLIC' }];
}
...
describe('PhotoManageView', () => {
  ...
  it('loads photos/albums and submits photo and album forms', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/admin/photos?page=1&pageSize=10', expect.any(Object));
    ...
    expect(JSON.parse(fetchMock.mock.calls.find(([url, init]) => url === '/api/admin/photos/sort' && init?.method === 'PUT')?.[1]?.body as string)).toEqual({
      items: [
        { id: 2, sortOrder: 0 },
        { id: 1, sortOrder: 1 },
      ],
    ...
    const vm = wrapper.vm as unknown as {
      ...
      photoForm: { originalUrl: string; title: string };
    };
    vm.openCreatePhotoDialog();
    vm.photoForm.title = '新照片';
    vm.photoForm.originalUrl = '/original-new.jpg';
    await vm.handleSubmitPhoto();
    await flushPromises();

    expect(JSON.parse(fetchMock.mock.calls.find(([url, init]) => url === '/api/admin/photos' && init?.method === 'POST')?.[1]?.body as string)).toMatchObject({
      originalUrl: '/original-new.jpg',
      title: '新照片',
    });
    ...
    expect(JSON.parse(fetchMock.mock.calls.find(([url, init]) => url === '/api/admin/albums' && init?.method === 'POST')?.[1]?.body as string)).toMatchObject({
      name: '新相册',
      slug: 'new-album',
    });
  });
