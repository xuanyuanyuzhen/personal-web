...
> 状态：**本文描述当前生效方案**（`Transition` / `page-shift`）。
> 前身「3D 书页翻页」已于本文方案落地时整体废弃并删代码，其设计存档见
> `doc/page-flip-3d-optimization.md`（仅供踩坑参考，不代表当前行为）。
> 范围：仅 `apps/web` 路由切换动效。
...
- 旧页面向上微滑 `6px` 并淡出；
- 新页面自下方 `10px` 轻盈上浮淡入；
- 过渡 `220–240ms`，使用 `mode="out-in"`，完整切换约 `460ms`；
- `prefers-reduced-motion: reduce` 下由全局规则将过渡压缩到近乎即时；
- 顶部导航、音乐播放器和看板娘不参与页面切换。

这套动效只使用 `opacity` 与 `transform`，没有克隆页面、冻结内容或额外覆盖层。

历史对照：早期曾做过左右 `12px` 位移（`190ms`），后为勘探「新页自下方上浮、旧页向上退场」的统一垂直语言，与站点内其他 `translateY(-6px)` 动效一致，且 `overflow-x: clip` 下无横向滚动风险。搜索页约定不受影响（见下）。
